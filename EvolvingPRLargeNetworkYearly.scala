...
val computeGraph = filteredGraph.mapVertices(v => Unit).pushDimension[Double](
  e => {
    val edge = e.data
    (edge.startDate.year.get to edge.endDate.getOrElse(LocalDate.now()).year.get).map(year => (year.toLong, 1.0))
  }, keepAllNodes = true
)
...
IOUtil.saveJsonLine(s"${dataDir}/evolving_pagerank_large_graph_yearly_experiment.jsonline", jsonLines)
